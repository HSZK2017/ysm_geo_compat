...
// CPU skinning fragment shader for converted YSM-GEO meshes (ported from the
// main project's cpu_skin.fsh). Identical semantics to the main project's
// GPU fragment shader (replicates Minecraft's entity shader: cutout/
// translucent alpha modes, overlay texelFetch, lightmap texelFetch, linear
// fog), only the GLSL version differs so the CPU render path runs on GPUs
// without compute-shader / SSBO support.
